...
use Closure;
use getpu\tree\widgets\Widget;
use getpu\tree\models\Tree;
use Yii;
...
HTML;
    ...
    protected function validateSourceData()
    {
        ...
        $trait = 'getpu\tree\models\TreeTrait';
    ...
    public function initOptions()
    {
        ...
        if (empty($this->searchClearOptions['title'])) {
            $this->searchClearOptions['title'] = Yii::t('tree', 'Clear search results');
        }
        ...
        if (!isset($this->searchOptions['placeholder'])) {
            $this->searchOptions['placeholder'] = Yii::t('tree', 'Search...');
        }
        $this->toolbarOptions['role'] = 'toolbar';
        $this->buttonGroupOptions['role'] = 'group';
        $this->clientMessages += [
            'invalidCreateNode' => Yii::t('tree', 'Cannot create node. Parent node is not saved or is invalid.'),
            'emptyNode' => Yii::t('tree', '(new)'),
            'removeNode' => Yii::t('tree', 'Are you sure you want to remove this node?'),
            'nodeRemoved' => Yii::t('tree', 'The node was removed successfully.'),
            'nodeRemoveError' => Yii::t('tree', 'Error while removing the node. Please try again later.'),
            'nodeNewMove' => Yii::t('tree', 'Cannot move this node as the node details are not saved yet.'),
            'nodeTop' => Yii::t('tree', 'Already at top-most node in the hierarchy.'),
            'nodeBottom' => Yii::t('tree', 'Already at bottom-most node in the hierarchy.'),
            'nodeLeft' => Yii::t('tree', 'Already at left-most node in the hierarchy.'),
            'nodeRight' => Yii::t('tree', 'Already at right-most node in the hierarchy.'),
            'emptyNodeRemoved' => Yii::t('tree', 'The untitled node was removed.'),
            'selectNode' => Yii::t('tree', 'Select a node by clicking on one of the tree items.'),
        ];
        $defaultToolbar = [
            self::BTN_CREATE => [
                'icon' => 'plus',
                'options' => ['title' => Yii::t('tree', 'Add new'), 'disabled' => true]
            ],
            self::BTN_CREATE_ROOT => [
                'icon' => $this->fontAwesome ? 'tree' : 'tree-conifer',
                'options' => ['title' => Yii::t('tree', 'Add new root')]
            ],
            self::BTN_REMOVE => [
                'icon' => 'trash',
                'options' => ['title' => Yii::t('tree', 'Delete'), 'disabled' => true]
            ],
            self::BTN_SEPARATOR,
            self::BTN_MOVE_UP => [
                'icon' => 'arrow-up',
                'options' => ['title' => Yii::t('tree', 'Move Up'), 'disabled' => true]
            ],
            self::BTN_MOVE_DOWN => [
                'icon' => 'arrow-down',
                'options' => ['title' => Yii::t('tree', 'Move Down'), 'disabled' => true]
            ],
            self::BTN_MOVE_LEFT => [
                'icon' => 'arrow-left',
                'options' => ['title' => Yii::t('tree', 'Move Left'), 'disabled' => true]
            ],
            self::BTN_MOVE_RIGHT => [
                'icon' => 'arrow-right',
                'options' => ['title' => Yii::t('tree', 'Move Right'), 'disabled' => true]
            ],
            self::BTN_SEPARATOR,
            self::BTN_REFRESH => [
                'icon' => 'refresh',
                'options' => ['title' => Yii::t('tree', 'Refresh')],
    ...
    public function renderRoot()
    {
        ...
        $content .= ArrayHelper::remove($this->rootOptions, 'label', Yii::t('tree', 'Root'));
