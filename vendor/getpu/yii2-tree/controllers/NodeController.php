...
namespace getpu\tree\controllers;
...
class NodeController extends Controller
{
    ...
    protected static function checkValidRequest($isInvalid = null)
    {
        ...
        if ($isInvalid) {
            throw new InvalidCallException(Yii::t('tree', 'This operation is not allowed.'));
        }
    }
    ...
    public function actionSave()
    {
        ...
        $modelClass = '\getpu\tree\models\Tree';
        ...
        if ($treeNodeModify) {
            $node = new $modelClass;
            $successMsg = Yii::t('tree', 'The node was successfully created.');
            $errorMsg = Yii::t('tree', 'Error while creating the node. Please try again later.');
        } else {
            $tag = explode("\\", $modelClass);
            $tag = array_pop($tag);
            $id = $post[$tag][$keyAttr];
            $node = $modelClass::findOne($id);
            $successMsg = Yii::t('tree', 'Saved the node details successfully.');
            $errorMsg = Yii::t('tree', 'Error while saving the node. Please try again later.');
        }
        ...
        if ($node->save()) {
            ...
            if (!empty($errors)) {
                ...
                foreach ($errors as $err) {
                    $errorMsg .= "<li>" . Yii::t('tree', "Node # {id} - '{name}': {error}", $err) . "</li>\n";
                }
                $errorMsg .= "</ul>";
            }
        } else {
    ...
    public function actionManage()
    {
        ...
        return self::process(
            $callback,
            Yii::t('tree', 'Error while viewing the node. Please try again later.'),
            null
        );
    }
    ...
    public function actionRemove()
    {
        ...
        return self::process(
            $callback,
            Yii::t('tree', 'Error removing the node. Please try again later.'),
            Yii::t('tree', 'The node was removed successfully.')
        );
    }
    ...
    public function actionMove()
    {
        ...
        $errorMsg = $isMovable ? Yii::t('tree', 'Error while moving the node. Please try again later.') :
            Yii::t('tree', 'The selected node cannot be moved.');
        ...
        return self::process($callback, $errorMsg, Yii::t('tree', 'The node was moved successfully.'));
    }
